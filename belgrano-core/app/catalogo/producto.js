    ...
    tabCostos() {
      ...
      const fila = v => {
        ...
        return `<div class="cv-r ${v.activa === false ? 'off' : ''}" data-v="${v.id}">
          ...
          <div class="cv-n"><input class="pn" inputmode="numeric" data-costo="${v.id}"
            value="${v.costo || ''}" placeholder="—" ${ed ? '' : 'readonly'}></div>
          <div class="cv-n"><input class="pn" inputmode="numeric" data-efec="${v.id}"
            value="${ef || ''}" placeholder="—" ${ed ? '' : 'readonly'}></div>
          <div class="cv-n"><span class="cv-lista tnum" data-lista="${v.id}">${UI.pesos(this.listaDe(ef))}</span></div>
          <div class="cv-n"><span class="tnum" data-marg="${v.id}">${mg ? mg.toFixed(1).replace('.', ',') + ' %' : '—'}</span></div>
          <div class="cv-n"><span class="mk ${banda.pill}" data-mk="${v.id}">${
            mk ? mk.toFixed(2).replace('.', ',') + 'x' : '—'}</span></div>
          <div><select data-medc="${v.id}" ${ed ? '' : 'disabled'}>${opts.map(m =>
            `<option ${norm(m) === act ? 'selected' : ''}>${UI.esc(m)}</option>`).join('')}</select></div>
        </div>`;
      };
      ...
      return `<section class="pd-b">
        ...
        <div class="vr-tools">
          ...
          <div class="sp"></div>
          <span class="hint">El de lista sale solo: efectivo ÷ ${(100 - global.DB.descuentoDe('efectivo'))}%</span>
        </div>
        <div class="cv-tabla" style="--ejes:${ejes.length}">
          <div class="cv-h">
            ...
            <span class="num">P. de lista</span><span class="num">Margen</span>
            <span class="num">Markup</span><span>Medida de costeo</span>
          </div>
    ...
    estilos() {
      return `<style>
        ...
        .cv-h,.cv-r{display:grid;min-width:940px;gap:8px;align-items:center;
          grid-template-columns:repeat(var(--ejes),minmax(140px,1fr)) 90px 90px 94px 68px 62px 96px}
