...
(function (global) {
  ...
  const Presupuesto = {
    ...
    totalFinal() { return this.total() + this.envioMonto() - this.descuentoExtra(); },
      ...
      m.innerHTML = `
        ...
        <div class="card cz-foot">
          <div class="cz-tyc">
            ...
            <textarea id="obs-ext" rows="1" placeholder="Lo que tiene que ver el cliente…">${UI.esc(this.obsExt)}</textarea>
            <div class="int-box">
              <div class="lbl">Comentarios internos
                <span class="muted">no salen impresas — para nosotros o producción</span></div>
              <textarea id="obs-int" rows="1" placeholder="Notas para producción, administración o para vos…">${UI.esc(this.obsInt)}</textarea>
            </div>
          </div>
          <div id="cz-tot"></div>
          <div class="cz-tyc tyc-full">
            <div class="lbl">Términos y condiciones
    ...
    pintarCliente() {
      ...
      c.innerHTML = `
        <div class="cz-cols">
          ...
          <div class="cz-col">
            ${f('c-tel', 'Teléfono', this.cli.telefono, '11 5555-2020', 'inputmode="tel"')}
    ...
    modalCalendario(desde, hasta, onlisto) {
      let d1 = desde || '', d2 = hasta || '';
      let mes = mesDe(d1 || enDias(0));
      const m = this.modal('', null, 380);
      ...
      const pintar = () => {
        ...
        caja.innerHTML = `
          <div class="row" style="margin-bottom:10px"><h3 style="color:var(--navy);margin:0;font-size:16px">Fechas de entrega</h3>
            <div class="sp" style="flex:1"></div><button class="lx" id="cal-x" style="font-size:20px">✕</button></div>
          <div class="cal-h">
          ...
          <div class="cal-pie">
            ...
            <div class="sp" style="flex:1"></div>
            <button class="btn primary" id="cal-ok" ${d1 && d2 ? '' : 'disabled'}>Listo</button>
          </div>`;

        document.getElementById('cal-x').onclick = () => m.remove();
        ...
        if (d1 && d2) ok.onclick = () => { m.remove(); onlisto(d1, d2); };
      };
      pintar();
    },
    ...
    pintar() {
      ...
      cont.querySelectorAll('[data-subt]').forEach(i => {
        i.oninput = () => { const n = i.value.replace(/[^\d]/g, ''); if (n !== i.value) i.value = n; };
        i.onchange = () => {
    ...
    pintarAdicionales() {
        ...
        <div class="cz-col">
          <div class="fr"><label>Entrega</label>
            <div class="fx"><span class="valf">${UI.esc(this.entregaTexto())}</span>
              <button class="lapiz" id="ad-cal" title="Elegir las fechas de entrega" aria-label="Elegir las fechas de entrega">✏️</button>
      ...
      document.getElementById('ad-cal').onclick = () => this.modalCalendario(a.desde, a.hasta, (d1, d2) => {
        a.desde = d1; a.hasta = d2; a.entregaTocada = true;
    ...
    pintarTotales() {
      ...
      t.innerHTML = `<div class="cz-tots">
        ...
        ${fila('Total a pagar', UI.pesos(this.totalFinal()), 'big')}
    ...
    reset() {
      ...
      this.lineas = []; this._uid = 0; this.dto = null;
      this.actividad = [];
    ...
    estilos() {
      return `<style>
        ...
        .cal-pie{display:flex;align-items:center;gap:12px;border-top:1px solid var(--line);margin-top:12px;padding-top:12px;font-size:12.5px}
        .rango{gap:7px}
        ...
        .tyc-full{grid-column:1/-1;border-top:1px solid var(--line);padding-top:12px;margin-top:2px}
        ...
        .tdto{display:flex;justify-content:space-between;align-items:center;gap:12px;width:100%;margin:6px 0 2px;
          border:1px dashed var(--line);background:var(--line-soft);border-radius:8px;padding:7px 10px;
