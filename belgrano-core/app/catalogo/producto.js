...
(function (global) {
  const Producto = {
    ...
    // Las columnas de la planilla de un rubro. Si tiene una PLANTILLA elegida,
    // las columnas salen de ahí: la de respaldos se usa en los 40 respaldos y
    // se edita en un solo lado. Si no, son propias de este mueble.
    planillaDe(r) {
      if (r && r.plantilla) {
    ...
    bloquePlanilla(r, i) {
      ...
      // Una columna de la plantilla que este mueble no tiene queda vacía —
      // igual que hoy en el papel, cuando a esa silla no hay que aclararle el
      // color del respaldo.
      const sinPropiedad = cols.filter(c => c.origen === 'propiedad' && !mias.includes(c.campo));

      return `<div class="pd-sepl"></div>
        <div class="ad-t">Planilla de pedido para ${UI.esc(this.nombreRubro(r.k))}</div>
        <div class="fx" style="margin-bottom:9px">
          ...
          ${ed ? `<button class="btn" data-guardarpl="${i}">＋ Guardar como planilla</button>` : ''}
        </div>
        ${t ? `<div class="banner info">Usa la planilla <b>${UI.esc(t.nombre)}</b>${usos > 1
            ? `, que comparten <b>${usos}</b> muebles` : ''}. Lo que cambies acá se cambia para
            ${usos > 1 ? 'todos ellos' : 'todos los que la usen'}.</div>`
          : '<div class="hint">Estas columnas valen sólo para este mueble. Guardala como planilla si la vas a repetir.</div>'}

        <div class="pl-cols" data-cols="${i}" style="margin-top:9px">
          ${cols.map((c, j) => {
            const huerfana = c.origen === 'propiedad' && !mias.includes(c.campo);
            return `<div class="pl-col ${huerfana ? 'vacia' : ''}" draggable="${ed}" data-col="${j}">
        ...
        ${sinPropiedad.length ? `<div class="hint" style="margin-top:8px">
          <b>${sinPropiedad.map(c => c.label).join(' · ')}</b>
          ${sinPropiedad.length === 1 ? 'no aplica' : 'no aplican'} a este mueble: ${
          sinPropiedad.length === 1 ? 'esa columna le queda vacía' : 'esas columnas le quedan vacías'},
          igual que en el papel.</div>` : ''}
      ...
      document.querySelectorAll('.pl-cols').forEach(caja => {
        ...
        caja.querySelectorAll('.pl-col').forEach(c => {
          ...
          c.ondragend = () => {
            ...
            const antes = cols(i);
            const nuevas = orden.map(j => antes[j]);
            const r = this.rubrosDe()[i];
            if (r.plantilla) {
              const t = global.DB.planilla(r.plantilla);
              if (t) global.DB.guardarPlanilla(t.nombre, nuevas, t.k);
            } else this.p.rubros[i].planilla = nuevas;
            this.guardar(); this.pintar();
          };
          c.ondragover = e => {
            e.preventDefault();
