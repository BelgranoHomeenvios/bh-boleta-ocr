  ...
  const Presupuesto = {
    ...
    pintar() {
      ...
      if (!this.lineas.length) {
        cont.innerHTML = UI.vacio('Buscá un mueble arriba para empezar la cotización.');
      ...
      cont.innerHTML = this.lineas.map(l => {
        ...
        const obs = medida ? `<div class="lobs">
        ...
        return `<div class="litem ${medida ? 'med' : ''}">
          <div class="lrow" data-l="${l.key}">
            <div class="lcant">
              <button class="vpm" data-menos="${l.key}" ${l.cantidad <= 1 ? 'disabled' : ''} aria-label="Quitar uno">−</button>
              <input type="number" min="1" value="${l.cantidad}" data-cant="${l.key}" aria-label="Cantidad">
              <button class="vpm" data-mas="${l.key}" aria-label="Sumar uno">+</button>
            </div>
            <div>${nombre}</div>
      ...
      const setCant = (l, n) => { if (l) { l.cantidad = Math.max(1, Math.floor(n) || 1); this.pintar(); } };
      cont.querySelectorAll('[data-cant]').forEach(i => i.onchange = () => setCant(this.get(i.dataset.cant), Number(i.value)));
      cont.querySelectorAll('[data-menos]').forEach(b => b.onclick = () => { const l = this.get(b.dataset.menos); setCant(l, l.cantidad - 1); });
      cont.querySelectorAll('[data-mas]').forEach(b => b.onclick = () => { const l = this.get(b.dataset.mas); setCant(l, l.cantidad + 1); });
    ...
    modalVariante(prod, vars) {
      ...
      let cant = 1;   // la cantidad se elige acá mismo, antes de los ejes
      ...
      const pintar = () => {
        ...
        caja.innerHTML = `
          ...
          <div class="vgrp"><div class="vlbl">Cantidad</div>
            <div class="vcant">
              <button class="vpm" id="mv-menos" ${cant <= 1 ? 'disabled' : ''} aria-label="Quitar uno">−</button>
              <input id="mv-cant" type="number" min="1" value="${cant}" aria-label="Cantidad">
              <button class="vpm" id="mv-mas" aria-label="Sumar uno">+</button>
            </div></div>
          ${ejes.map(e => {
        ...
        const setCant = n => { cant = Math.max(1, Math.floor(n) || 1); pintar(); };
        document.getElementById('mv-menos').onclick = () => setCant(cant - 1);
        document.getElementById('mv-mas').onclick = () => setCant(cant + 1);
        document.getElementById('mv-cant').onchange = e => setCant(Number(e.target.value));
    ...
    estilos() {
      return `<style>
        ...
        .srow{display:grid;grid-template-columns:104px minmax(0,1.6fr) auto;gap:8px;align-items:center;padding:10px 0}
        ...
        .lhead,.lrow{display:grid;grid-template-columns:104px minmax(0,1.6fr) 92px 100px 108px 108px 24px;gap:8px;align-items:center;padding:0}
        .lcant{display:flex;align-items:center;gap:3px}
        .lcant input{width:38px;text-align:center;padding:5px 2px}
        .lcant .vpm{width:24px;height:24px;font-size:13px}
        .lrow .dto{text-align:right;font-size:12.5px;color:var(--ok)}
        .litem{border-bottom:1px solid var(--line-soft)}
        .litem.med{background:var(--panel-2);border-radius:8px;padding:0 8px;margin:2px -8px}
        ...
        .lobs{padding:0 0 11px 112px;display:flex;flex-direction:column;gap:7px}
        ...
        @media(max-width:1000px){.lhead{display:none}.lrow{grid-template-columns:104px 1fr 1fr;gap:8px}.lrow .sub,.lrow .lock,.lrow .pnum{text-align:left}}
