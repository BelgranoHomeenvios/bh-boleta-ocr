...
(function (global) {
  ...
  // Cómo llega el mueble. Decide qué domicilio se pregunta y si hay flete que
  // cotizar: retirando no hay envío; por expreso el flete es hasta el expreso;
  // "a confirmar" deja todo cargable pero sin exigirlo.
  const ENTREGAS = [
    { k: 'envio',     label: 'Envío CABA o GBA' },
    { k: 'retira',    label: 'Retira cliente' },
    { k: 'expreso',   label: 'Expreso' },
    { k: 'confirmar', label: 'A confirmar' },
  ];

  // Estado de cada mueble DENTRO DE LA ORDEN. En la cotización no se pregunta:
  // todavía no se sabe cuándo va a comprar el cliente.
  const ESTADOS = [
    { k: 'fabricar',   label: 'A fabricar',    pill: '',    nota: 'va a fabricar' },
    { k: 'produccion', label: 'En producción', pill: 'wip', nota: 'ya está en producción' },
    { k: 'listo',      label: 'Listo',         pill: 'ok',  nota: 'sale de stock' },
  ];
  ...
  const Presupuesto = {
    ...
    envioMonto() { return this.pideEnvio() ? (this.ad.envio || 0) : 0; },
    ...
    // Si nada va a fábrica, no hay plazo de fabricación que esperar. En la
    // cotización el estado ni se pregunta, así que el plazo siempre corre.
    verEstado() { return this.esOrden(); },
    todoListo() { return this.verEstado() && !!this.lineas.length && this.lineas.every(l => l.estadoProd === 'listo'); },
    ...
    // Retirando no hay flete. Con envío a CABA/GBA el costo sale solo de la
    // localidad; por expreso o a confirmar se puede cargar a mano, pero no
    // se inventa un número.
    entregaModoLabel() { return (ENTREGAS.find(o => o.k === this.cli.entrega) || ENTREGAS[0]).label; },
    pideEnvio() { return this.cli.entrega !== 'retira'; },
    envioCotizable() { return this.cli.entrega === 'envio' && !!(String(this.cli.domicilio).trim() && this.cli.localidad); },
    // El flete se puede escribir a mano siempre que haya algo que enviar.
    envioCargable() { return this.pideEnvio(); },
    envioTexto() {
      if (!this.pideEnvio()) return 'Sin envío — retira';
      if (this.envioCotizable() || this.ad.envioTocado) return UI.pesos(this.ad.envio);
      return global.DB.ENVIO_SIN_DOMICILIO;
    },
    instalacionTexto() {
    ...
    resumen(k) {
      if (k === 'cliente') {
        ...
        if (!partes.length) return '<i>sin cargar</i>';
        return UI.esc(partes.concat(this.entregaModoLabel()).join(' · '));
      }
      if (k === 'productos') {
        const n = this.items();
      ...
      // La sugerencia no va acá arriba: vive al lado del monto recibido.
      const x = this.senaTotal();
      return x ? `seña ${UI.pesos(x)} · saldo ${UI.pesos(this.saldo())}` : '<i>sin seña</i>';
    },
    ...
    pintarCliente() {
      ...
      // La forma de entrega decide qué domicilio hace falta: si retira no se
      // pregunta ninguno, si va por expreso se agrega el del expreso, y con
      // "a confirmar" los campos quedan disponibles pero sin exigirse.
      const ent = this.cli.entrega;
      const pideDom = ent !== 'retira';
      const opc = ent === 'confirmar' || ent === 'expreso' ? ' <span class="opt">(opcional)</span>' : '';

      const campoLoc = !pideDom ? '' : `
        <div class="fr"><label for="c-locd">Localidad${opc}</label>
          <div class="fx locw">
      ...
      const campoDom = !pideDom ? '' : `
        <div class="fr"><label for="c-dom">Domicilio de entrega${opc}</label>
          <input id="c-dom" value="${UI.esc(this.cli.domicilio)}" placeholder="Calle 1234, piso/depto"></div>`;

      c.innerHTML = `
        <div class="cz-cols">
          <div class="cz-col">
            <div class="fr"><label for="c-nombre">Cliente ${mas('instagram', '+ IG')}</label>
              <input id="c-nombre" value="${UI.esc(this.cli.nombre)}" placeholder="Nombre y apellido"></div>
            ${opcional('instagram')}
        ...
        <div class="cz-cols">
          <div class="cz-col">
            <div class="fr"><label for="c-entrega">Forma de entrega</label>
              <select id="c-entrega">${ENTREGAS.map(o =>
                `<option value="${o.k}" ${ent === o.k ? 'selected' : ''}>${UI.esc(o.label)}</option>`).join('')}</select></div>
            ${ent === 'confirmar' ? `<div class="fr"><label></label><span class="hint">Se define después: el envío se puede cargar igual o dejarlo a confirmar.</span></div>` : ''}
            ${campoDom}
            ${ent === 'expreso' ? `<div class="fr"><label for="c-expdom">Domicilio del expreso <span class="opt">(opcional)</span></label>
              <input id="c-expdom" value="${UI.esc(this.cli.expresoDom)}" placeholder="Se completa después"></div>` : ''}
          </div>
          <div class="cz-col">
            ${campoLoc}
            ${ent === 'expreso' ? `<div class="fr"><label for="c-expreso">Expreso <span class="opt">(opcional)</span></label>
              <input id="c-expreso" value="${UI.esc(this.cli.expreso)}" placeholder="Nombre del expreso"></div>` : ''}
          </div>
        </div>
      ...
      bind('c-expreso', 'expreso'); bind('c-expdom', 'expresoDom');
      ...
      // La forma de entrega mueve el flete: si retira no se cotiza y si es
      // expreso o a confirmar el costo deja de salir solo. Es un cambio con
      // impacto, así que queda registrado.
      document.getElementById('c-entrega').onchange = ev => {
        const antes = this.antes();
        this.cli.entrega = ev.target.value;
        if (!this.pideEnvio()) { this.ad.envio = 0; this.ad.envioTocado = false; this.ad.envioAbierto = false; }
        else if (this.envioCotizable() && !this.ad.envioTocado) this.ad.envio = global.DB.fleteDe(this.cli.localidad);
        this.cambio(`Cambió la forma de entrega a ${this.entregaModoLabel()}`, antes);
        this.pintarTodo();
      };
      ...
      const ORDEN = ['c-nombre', 'c-tel', 'c-canal', 'c-mail', 'c-entrega', 'c-dom', 'c-locd', 'c-expdom', 'c-expreso']
    ...
    pintarProductos() {
      ...
      cont.innerHTML = `
        ...
        <div class="lhead ${this.verEstado() ? '' : 'noest'}" id="pr-head"></div>
        <div id="pr-lineas"></div>
        <div class="srow ${this.verEstado() ? '' : 'noest'}">
          ...
          <span></span><span></span><span></span>${this.verEstado() ? '<span></span>' : ''}
        </div>
        <div class="pr-pie">
    ...
    pintar() {
      ...
      if (head) {
        head.classList.toggle('noest', !this.verEstado());
        head.innerHTML = `<span>Cant.</span><span>Producto</span><span>Tipo</span>
          ${this.verEstado() ? '<span>Estado</span>' : ''}
      ...
      cont.innerHTML = this.lineas.map(l => {
        ...
        // El estado (listo / en producción / a fabricar) es de la ORDEN: cuando
        // se cotiza todavía no se sabe cuándo va a comprar el cliente.
        const est = ESTADOS.find(x => x.k === l.estadoProd) || ESTADOS[0];
        const estCel = !this.verEstado() ? '' : `<div>${abierta
          ? `<select class="est" data-est="${l.key}">${ESTADOS.map(x =>
              `<option value="${x.k}" ${est.k === x.k ? 'selected' : ''}>${UI.esc(x.label)}</option>`).join('')}</select>`
          : `<span class="est fijo ${UI.esc(est.pill)}">${UI.esc(est.label)}</span>`}</div>`;
        return `<div class="litem ${medida ? 'med' : ''} ${abierta ? 'abierta' : ''}">
          <div class="lrow ${this.verEstado() ? '' : 'noest'}" data-l="${l.key}">
      ...
      cont.querySelectorAll('[data-est]').forEach(sl => sl.onchange = () => {
        ...
        const e = ESTADOS.find(x => x.k === l.estadoProd) || ESTADOS[0];
        this.log(this.vendedor, `${l.prodNombre}: ${e.nota}`);
    ...
    pintarAdicionales() {
      ...
      c.innerHTML = `<div class="cz-cols">
        <div class="cz-col">
          ...
          ${this.todoListo()
            ? `<div class="fr"><label></label><span class="hint" id="ad-plazo">Todo sale de <b>stock</b>: no hay plazo de fabricación, se coordina la entrega.</span></div>`
            : ''}
          ...
          ${!this.pideEnvio()
            ? `<div class="fr"><label>Costo de envío</label>
                 <span class="valf">Sin envío — retira</span></div>
               <div class="fr"><label></label><span class="hint">El cliente <b>retira</b>: no se cotiza flete.</span></div>`
            : a.envioAbierto
              ? `<div class="fr"><label for="ad-envio">Costo de envío</label>
                   <div class="fx"><input id="ad-envio" inputmode="numeric" value="${a.envio}">
                     <button class="lapiz" data-cerrar="envio" title="${hayDom ? 'Volver al de la localidad' : 'Volver a “a confirmar”'}" aria-label="Volver">↺</button></div></div>`
              : hayDom
                ? cerrado('Costo de envío', UI.pesos(a.envio), 'envio',
                    `Por la localidad <b>${UI.esc(locd)}</b>.`, '🔀')
                : a.envioTocado
                  ? cerrado('Costo de envío', UI.pesos(a.envio), 'envio', 'Cargado a mano.', '🔀')
                  : cerrado('Costo de envío', DB.ENVIO_SIN_DOMICILIO, 'envio',
                      this.cli.entrega === 'expreso'
                        ? 'El flete hasta el <b>expreso</b> se carga a mano.'
                        : 'Se puede cargar a mano o dejarlo a confirmar.', '🔀')}
        </div>
    ...
    // Qué porcentaje del total representa lo que está por señar (contando lo
    // que ya señó), para que se vea en el momento si llega al mínimo.
    pctNum(monto) {
      const t = this.totalFinal();
      if (!t) return 0;
      return ((this.senaTotal() + (Number(monto) || 0)) / t) * 100;
    },
    pctSena(monto) {
      if (!this.totalFinal()) return '—';
      const p = this.pctNum(monto);
      return `${p % 1 ? p.toFixed(1).replace('.', ',') : p.toFixed(0)}%`;
    },
    ...
    pintarSena() {
      ...
      // El total, lo señado y el saldo ya están en el cuadro de la derecha:
      // repetirlos acá sólo hace ruido.
      c.innerHTML = `
        ...
        <div class="cz-cols">
          <div class="cz-col">
            ...
            <div class="fr"><label for="sn-monto">Monto recibido</label>
              <div class="fx"><input id="sn-monto" class="sn-monto" inputmode="numeric" value="${n.monto}">
                <span class="sn-pct tnum" id="sn-pct">${this.pctSena(n.monto)}</span></div></div>
          ...
          <div class="cz-col">
            ${transfer
              ...
              : `<div class="fr"><label></label><span class="hint">El efectivo entra a la caja del local <b>y</b> del vendedor
                   —la plata del local la tiene en mano un vendedor—; <b>a quién se le rindió se resuelve en el arqueo</b>, no acá.</span></div>`}
          </div>
        </div>
      ...
      // Mientras escribe el monto, al lado se ve qué porcentaje del total está
      // señando: así sabe en el momento si llega al mínimo o no.
      mo.oninput = () => {
        const x = mo.value.replace(/[^\d]/g, ''); if (x !== mo.value) mo.value = x;
        n.monto = Number(x) || 0;
        const p = document.getElementById('sn-pct');
        if (p) { p.textContent = this.pctSena(n.monto); p.classList.toggle('ok', this.pctNum(n.monto) >= global.DB.SENA_PCT); }
      };
      mo.oninput();
    ...
    convertir() {
      ...
      const nro = global.DB.proximoNumeroOrden(this.local);
      const nombreCli = this.nombreCli();
      // Nada de resumen: se confirma y se pasa a la orden, que es donde se
      // termina de cargar lo que en la cotización todavía no importaba.
      this.modal(`<h3 style="color:var(--navy);margin:0 0 6px">¿Confirmamos la orden ${UI.esc(nro)}?</h3>
        <p class="muted" style="font-size:13px;margin:0">
          La cotización ${UI.esc(global.DB.numeroCotizacion(this.nro))} de <b>${UI.esc(nombreCli)}</b>
          pasa a orden de venta y se abre para completarla.</p>
        <div class="row" style="margin-top:18px;justify-content:flex-end;gap:10px">
          <button class="btn" id="cv-no">No, seguir editando</button>
          <button class="btn primary" id="cv-si">Sí, confirmar</button></div>`, m => {
        document.getElementById('cv-no').onclick = () => m.remove();
        document.getElementById('cv-si').onclick = () => {
          const orden = global.DB.crearOrden({
            cliente: nombreCli, vendedor: this.vendedor, local: this.local, total: this.totalFinal(),
    ...
    reset() {
      ...
      this.cli = { nombre: '', telefono: '', email: '', instagram: '', dni: '', tel2: '', canal: '',
        domicilio: '', localidad: '', factura: 'no',
        entrega: 'envio', expreso: '', expresoDom: '' };
    ...
    // ---- Presupuesto A4 (lo que ve y recibe el cliente) ------------------
    cuerpoPreview() {
      const a = this.ad, DB = global.DB, c = this.cli, E = DB.EMPRESA;
      const contacto = [c.telefono, c.tel2, c.email, c.instagram].filter(Boolean).join(' · ');
      const dom = [c.domicilio, (DB.localidades().find(l => l.k === c.localidad) || {}).label]
        .filter(Boolean).join(' — ');
      const lista = this.totalLista(), desc = this.descuento(), extra = this.descuentoExtra();
      // El número va espaciado, como en el presupuesto impreso: 0 0 1 4 7
      const nro = String(this.nro).padStart(5, '0').split('').join(' ');

      const dato = (l, v) => `<div class="pv-dato"><span>${UI.esc(l)}</span> <b>${UI.esc(v)}</b></div>`;
      const cond = (l, v) => `<div class="pv-cond"><span>${UI.esc(l)}</span> · <b>${UI.esc(v)}</b></div>`;
      const tot = (l, v, cls = '') => `<div class="pv-tr ${cls}"><span>${UI.esc(l)}</span><b>${UI.esc(v)}</b></div>`;

      return `<div class="pv">
        <div class="pv-top"></div>

        <header class="pv-head">
          <div>
            <div class="pv-logo">${UI.esc(E.nombre)}<span>.</span></div>
        ...
        <div class="pv-cli">
          ...
          ${dato('Entrega:', this.entregaModoLabel())}
          ${this.pideEnvio() && dom ? dato('Domicilio:', dom) : ''}
          ${c.entrega === 'expreso' && (c.expreso || c.expresoDom)
            ? dato('Expreso:', [c.expreso, c.expresoDom].filter(Boolean).join(' — ')) : ''}
          ...
          <tbody>${this.lineas.map(l => {
            ...
            return `<tr>
              <td><b>${UI.esc(l.prodNombre || 'Mueble a medida')}</b>
                ${pie ? `<div class="pv-var">${UI.esc(pie)}</div>` : ''}</td>
              <td class="num">${l.cantidad}</td>
              <td class="num">${sin ? 'A definir' : UI.pesos(u)}</td>
              <td class="num fuerte">${sin ? '—' : UI.pesos(sub)}</td></tr>`;
          }).join('')}</tbody>
        </table>

        <div class="pv-band">
          <div class="pv-conds">
            ${cond('Tiempo de entrega', this.entregaTexto())}
            ${cond('Instalación', this.instalacionMonto() ? UI.pesos(this.instalacionMonto()) : this.instalacionTexto())}
            ${cond('Saldo se abona en', a.saldoEn)}
            ${cond('Subida por escalera', DB.escaleraTexto(a.escalera))}
            ${cond('Seña', DB.SENA_LEYENDA)}
            ${cond('IVA', DB.IVA_LEYENDA.replace(/\.$/, ''))}
          </div>
          <div class="pv-tots">
            ${tot('Muebles (lista)', UI.pesos(lista))}
            ...
            ${this.pideEnvio() ? tot('Envío', this.envioTexto()) : tot('Envío', 'Retira')}
            <div class="pv-total"><span>Total</span><b>${UI.esc(UI.pesos(this.totalFinal()))}</b></div>
          </div>
        </div>

        <div class="pv-obs">${this.obsExt ? UI.esc(this.obsExt) : ''}</div>

        <footer class="pv-pie">
          <div class="pv-tyc">${UI.esc(DB.TYC_DEFAULT)}</div>
          <div class="pv-marca">
            <b>${UI.esc(E.nombre)}</b>
            <span>${UI.esc(E.direccion)}</span>
            <span>${UI.esc(E.instagram)}</span>
            <span class="tel">${UI.esc(E.telefono)}</span>
          </div>
        </footer>
      </div>`;
    ...
    // Hoja A4 en blanco y negro, con el sello de la marca. Mismo HTML para ver,
    // imprimir y descargar.
    estiloPreview() {
      return `<style>
        @page{size:A4;margin:0}
        .pv{--tinta:#141414;--gris:#8b8b8b;--linea:#dcdcdc;--tierra:#8a5a3c;
          box-sizing:border-box;width:100%;max-width:210mm;min-height:297mm;margin:0 auto;
          background:#fff;color:var(--tinta);padding:16mm 15mm 10mm;
          font-family:'Segoe UI',-apple-system,system-ui,Helvetica,Arial,sans-serif;
          font-size:11px;line-height:1.45;display:flex;flex-direction:column}
        .pv *{box-sizing:border-box}
        .pv-top{border-top:1px solid var(--linea);width:46%;margin-bottom:11mm}

        .pv-head{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;
          padding-bottom:5mm;border-bottom:1.6px solid var(--tinta)}
        .pv-logo{font-size:30px;font-weight:600;letter-spacing:-.6px;line-height:1}
        .pv-logo span{color:var(--tinta)}
        .pv-lema{margin-top:7mm;color:var(--tierra);font-size:9.5px;font-weight:700;
          letter-spacing:.22em;text-transform:uppercase}
        .pv-meta{text-align:right;display:flex;flex-direction:column;gap:5px;white-space:nowrap}
        .pv-meta div{display:flex;justify-content:flex-end;align-items:baseline;gap:14px}
        .pv-meta span{color:var(--gris);font-size:9.5px;letter-spacing:.14em;text-transform:uppercase}
        .pv-meta b{font-size:12px}
        .pv-meta .esp{font-size:14px;letter-spacing:.12em}

        .pv-cli{padding:6mm 0 0}
        .pv-dato{font-size:12.5px;margin-bottom:3.4mm}
        .pv-dato span{color:var(--gris)}
        .pv-dato b{color:var(--tinta)}

        .pv-sec{margin:4mm 0 3mm;font-size:9.5px;font-weight:700;letter-spacing:.22em;text-transform:uppercase}
        .pv-tabla{width:100%;border-collapse:collapse}
        .pv-tabla th{font-size:9px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;
          color:var(--gris);text-align:right;padding:0 0 2.4mm;border-bottom:1px solid var(--linea)}
        .pv-tabla th:first-child{text-align:left}
        .pv-tabla td{padding:3.2mm 0;border-bottom:1px solid var(--linea);vertical-align:top}
        .pv-tabla td.num{text-align:right;white-space:nowrap;padding-left:8mm;
          font-variant-numeric:tabular-nums}
        .pv-tabla td b{font-size:12.5px;font-weight:700}
        .pv-tabla td.fuerte{font-weight:700;font-size:12.5px}
        .pv-var{color:var(--gris);font-size:10.5px;margin-top:1mm}

        .pv-band{display:flex;gap:10mm;margin-top:7mm;align-items:flex-start}
        .pv-conds{flex:1;padding-top:1mm}
        .pv-cond{font-size:11px;margin-bottom:2.6mm;color:var(--gris)}
        .pv-cond b{color:var(--tinta)}
        .pv-tots{width:72mm;flex:none}
        .pv-tr{display:flex;justify-content:space-between;align-items:baseline;gap:10px;
          padding:2.9mm 4mm;border:1px solid var(--linea);border-bottom:0;font-size:11.5px;color:var(--gris)}
        .pv-tr b{color:var(--tinta);font-weight:700;font-variant-numeric:tabular-nums}
        .pv-tr.neg b{color:var(--tierra)}
        .pv-total{display:flex;justify-content:space-between;align-items:center;gap:10px;
          background:var(--tinta);color:#fff;padding:4mm}
        .pv-total span{font-size:9.5px;font-weight:700;letter-spacing:.2em;text-transform:uppercase}
        .pv-total b{font-size:20px;font-weight:700;font-variant-numeric:tabular-nums}

        .pv-obs{flex:1;min-height:26mm;border:1px solid var(--linea);margin-top:7mm;padding:4mm;
          font-size:11px;color:var(--tinta);white-space:pre-wrap}
        .pv-pie{margin-top:5mm}
        .pv-tyc{color:var(--gris);font-size:9.5px;line-height:1.6;max-width:150mm}
        .pv-marca{display:flex;align-items:baseline;gap:5mm;flex-wrap:wrap;margin-top:3.5mm;
          padding-top:3mm;border-top:1px solid var(--linea);font-size:10.5px;color:var(--gris)}
        .pv-marca b{color:var(--tinta);font-size:11.5px}
        .pv-marca .tel{color:var(--tinta);font-weight:600}
      </style>`;
    },
    ...
    estilos() {
      return `<style>
        ...
        /* En la cotización no hay columna Estado: la grilla va sin esa franja. */
        .lhead.noest,.lrow.noest{grid-template-columns:56px minmax(0,1.4fr) 116px 96px 90px 100px 48px}
        ...
        .est.fijo.wip{color:var(--brand);background:var(--brand-soft)}
        ...
        /* El monto va corto y al lado, en vivo, qué % del total se está señando. */
        .sn-monto{max-width:150px}
        .sn-pct{flex:none;font-size:13px;font-weight:700;color:var(--muted);
          border:1px solid var(--line);border-radius:7px;padding:6px 9px;background:var(--panel-2);min-width:56px;text-align:center}
        .sn-pct.ok{color:var(--ok);border-color:var(--ok);background:var(--ok-bg,#e9f7f0)}
